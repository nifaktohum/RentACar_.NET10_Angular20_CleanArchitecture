...
// Stokta olan ürünleri veritabanından çeker.
public sealed record GetInStockExtrasQuery : IRequest<Result<List<ExtraSummaryDto>>>;

public sealed class GetInStockExtrasQueryHandler(
                            IExtraRepository _extraRepo,
                            IUserRepository _userRepo
                    ) : IRequestHandler<GetInStockExtrasQuery, Result<List<ExtraSummaryDto>>>
{
  public async Task<Result<List<ExtraSummaryDto>>> Handle(GetInStockExtrasQuery _req, CancellationToken _token)
  {
    var extras = await _extraRepo.GetInStockAsync(_token);
    ...
    var extralist = extras.Select(e => new ExtraSummaryDto(
        Id: e.Id,
        Name: e.Name,
        Price: e.Price,
        Icon: e.Icon,
    ...
    return Result<List<ExtraSummaryDto>>.Succeed(extralist);
  }
}